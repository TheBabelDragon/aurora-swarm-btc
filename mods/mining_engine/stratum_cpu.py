"""Official CPU stratum miner — reconnect, quiet log, niceness."""
...
def _parse_pool(url: str) -> Tuple[str, int]:
    ...
    return u.hostname or "solo.stratum.braiins.com", int(u.port or 3333)
...
def _diff_to_target(diff: float) -> int:
    ...
    return max(1, int(max_target / diff))


def _hash_process(tid, step, stop_flag, hash_counter, prefix_buf, prefix_ready, target_buf, hits):
    try:
        os.nice(int(os.getenv("AURORA_MINE_NICE", "10") or 10))
    except Exception:
        pass
    ...
    while stop_flag.value == 0:
        if prefix_ready.value == 0:
            ...
            tgt = int.from_bytes(bytes(target_buf), "big")
            if tgt and int.from_bytes(digest[::-1], "big") <= tgt:
                try:
...
class StratumCpuMiner:
    def __init__(self, pool_url, username, password="x", threads=0, line_queue=None, comms=None, coin="BTC"):
        self.pool_url = pool_url
        self.username = username
        self.password = password
        cpus = os.cpu_count() or 2
        self.offline = os.getenv("AURORA_MINE_OFFLINE", "0") in ("1", "true", "True")
        self.threads = max(1, threads if threads and threads > 0 else cpus)
        self.lines = line_queue if line_queue is not None else queue.Queue(maxsize=500)
        self.comms = comms
        self.coin = coin
        self._sock = None
        ...
        self._prefix_ready = Value("i", 1 if self.offline else 0)
    ...
    def _emit(self, line: str, kind: str = "info", persist: bool = True):
        try:
    ...
    def _apply_subscribe(self, res: Any) -> bool:
        ...
        self._emit(f"subscribed en1_len={len(self._extranonce1)} en2={self._extranonce2_size}")
        return True

    def connect(self) -> bool:
        if self.offline:
            self._emit("OFFLINE hasher — not sending to a pool")
            return True
        host, port = _parse_pool(self.pool_url)
        try:
            self._sock = socket.create_connection((host, port), timeout=12)
            self._sock.settimeout(20)
            self._rpc("mining.subscribe", ["aurora-official/1.1"])
            self._rpc("mining.authorize", [self.username, self.password])
            self._emit(f"connected {host}:{port} user={self.username}")
            self.last_error = ""
            return True
        except Exception as e:
            self.last_error = str(e)
            self._emit(f"connect failed {host}:{port} {e}", "error")
            self._sock = None
            return False

    def _set_target_bytes(self, target: int):
        raw = int(target).to_bytes(32, "big")
        if self._target_buf is not None:
            for i, b in enumerate(raw):
                self._target_buf[i] = b
    ...
    def _rebuild_prefix(self):
        ...
        try:
            ...
            self._set_target_bytes(self._target)
            ...
            self._header_prefix = None
            self.job_ready = False
            self._emit(f"prefix rebuild failed: {e}", "error")
        ...
        if method == "mining.notify":
            ...
            if len(params) >= 8:
                with self._job_lock:
                    ...
                    self._rebuild_prefix()
                if self.job_ready:
                    self._emit(f"job {str(params[0])[:16]} diff={self._difficulty} ready")
                    self.last_error = ""
            ...
            try:
                ...
                with self._job_lock:
                    self._rebuild_prefix()
                self._emit(f"difficulty {self._difficulty}")
            except Exception:
                pass
        elif mid is not None:
            if mid in self._pending:
                nonce = self._pending.pop(mid)
                if msg.get("error"):
                    self.shares_rejected += 1
                    self._emit(f"share REJECTED nonce={nonce:08x} {msg.get('error')}", "warn")
                elif msg.get("result") is True:
                    self.shares_accepted += 1
                    self._emit(f"share ACCEPTED nonce={nonce:08x} → {self.username}", "ok")
                else:
                    self.shares_rejected += 1
                    self._emit(f"share rejected nonce={nonce:08x} result={msg.get('result')}", "warn")
                return
            if msg.get("error"):
                self.last_error = str(msg.get("error"))
                self._emit(f"rpc error {self.last_error}", "error")
                return
            res = msg.get("result")
            if res is True:
                self.authorized = True
                self._emit(f"authorized {self.username}")
                return
            if res is False:
                self.last_error = "authorize rejected — wallet/user not accepted by pool"
                self._emit(self.last_error, "error")
                return
        ...
        while not self._stop.is_set():
            try:
                ...
                if not chunk:
                    ...
                    self._emit("pool connection closed", "warn")
                    break
    ...
    def _watch_loop(self):
        ...
        while not self._stop.is_set():
            ...
            if self._sock is not None:
                backoff = 3.0
                continue
            self._emit(f"reconnecting in {backoff:.0f}s", "warn")
            if self.connect():
    ...
    def _submit_loop(self):
        ...
        while not self._stop.is_set():
            ...
            if not job_id or ntime is None or self._sock is None:
                continue
            try:
                mid = self._rpc(
                    "mining.submit",
                ...
                self._pending[mid] = int(nonce)
                self.shares_submitted += 1
                self._emit(f"share SUBMIT nonce={int(nonce):08x} job={job_id}")
            except Exception as e:
                self._emit(f"submit failed {e}", "error")
    ...
    def _hr_loop(self):
        while not self._stop.is_set():
            ...
            if rate >= 1e6:
                shown = f"{rate/1e6:.2f} MH/s"
            elif rate >= 1e3:
                shown = f"{rate/1e3:.2f} KH/s"
            else:
                shown = f"{rate:.0f} H/s"
            persist = (now - self._last_hr_log) >= 30
            if persist:
                self._last_hr_log = now
            self._emit(
                f"hashrate {shown} acc={self.shares_accepted} rej={self.shares_rejected} sub={self.shares_submitted}",
                persist=persist,
            )
    ...
    def _spawn_workers(self):
        ...
        for i in range(self.threads):
            p = Process(
                ...
                args=(
                    i, self.threads, self._stop_flag, self._hash_counter,
                    self._prefix_buf, self._prefix_ready, self._target_buf, self._hits,
                ),
                name=f"cpu-mine-{i}",
                daemon=True,
            )
        ...
        self._emit(f"hasher processes={self.threads} cores={os.cpu_count()} nice={os.getenv('AURORA_MINE_NICE', '10')}")
    ...
    def start(self) -> bool:
        ...
        if not self.offline:
            ...
            self._watch = threading.Thread(target=self._watch_loop, name="stratum-watch", daemon=True)
            self._watch.start()
        self._hr_thread = threading.Thread(target=self._hr_loop, name="stratum-hr", daemon=True)
        self._hr_thread.start()
        return True if self.offline else True
    ...
    def running(self) -> bool:
        if self._stop.is_set():
            return False
        return any(p.is_alive() for p in self._workers) or self._sock is not None
